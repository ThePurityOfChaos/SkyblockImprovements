...
public class InventoryProcessor {
    ...
    public static Map<Text,AbstractMap.SimpleEntry<Integer,ComponentMap>>processListToMap(List<ItemStack> list){
        SkyblockImprovements.push("SBI_processListToMap");
        Map<Text,AbstractMap.SimpleEntry<Integer,ComponentMap>> map = new HashMap<>();
        for(int i=0; i<list.size(); i++){
            ...
            try{
                ...
                if(item!=null){
                    ...
                    ComponentMap data = item.getComponents();
                    ...
                    if(!item.getName().getString().contains(" x"))
                        map.put(item.getName(), new AbstractMap.SimpleEntry<Integer,ComponentMap>(count,data));
                    else{
                        ...
                        map.put(merchantlessItem, new AbstractMap.SimpleEntry<Integer,ComponentMap>(count,data));
                    }
                }
            } catch(Exception e){
