...
public class GeneratorScreen extends MenuScreen {
    ...
    public static boolean parseItemToGenerator(ItemStack item){
        ...
        //parse
        ComponentMap itemData = item.getComponents();
        itemData.getTypes();
        try{
            //set name
            screenInstance.name.setText(ComponentUtils.getStringFromName("temp"));
            //get rarity and type information (recomb included as rarity + rarity.length)
            Pair<Integer,String> rarityAndType = ComponentUtils.getRarityAndTypeFromLore("temp");
            //set rarity
            ...
            //set generator body
            screenInstance.generator.setText(Utils.getColorString('f')+ComponentUtils.getStringFromLore("temp", rarityAndType.getLeft()==0)+" ");
            return true;
        }catch(Exception e){
            return false;
        }
