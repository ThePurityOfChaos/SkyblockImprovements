...
public class PackSwapper extends Feature {
    ...
    public void manipulatePacks(String eArea, String eRegion){
        ...
        for(ResourcePackProfile pack:packs){
            ...
            if(!pack.getDescription().getString().toLowerCase().contains("sbi compatible")){
                if(pack.isPinned()||pack.isRequired()){
                    guaranteedPacks.add(pack.getId());
                }
                if(currentlyEnabledPacks.contains(pack))
                    unmodifiedPacks.add(pack.getId());
                continue;
            }

            //if the pack is new, add it to the main pack map.
            if(!packAreaRegionToggles.containsKey(pack.getId())){
